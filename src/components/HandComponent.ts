...
export class HandComponent extends HTMLElement {
  //aqui diz pro navegador tratar essa classe como um componente HTML
  ...
  constructor() {
    ...
    this.shadowRoot?.addEventListener(
      "card-selected",
      this.handleCardSelection as EventListener
    );
...
`;
    ...
    this._hand.getCards().forEach((card) => {
      ...
      const cardElement = document.createElement("ice-card") as CardComponent;
      ...
      cardElement.cards = card;
      //define se a carta deve aparecer selecionada
      cardElement.isSelected = this._selectedCards.includes(card);
